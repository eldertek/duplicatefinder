<template>
  <div id="duplicatefinder_settings_form">
    <NcSettingsSection :name="t('duplicatefinder', 'Duplicate Finder Settings')"
      :description="t('duplicatefinder', 'All general settings to modify Duplicate Finder behaviors.')" :limit-width="true">
    </NcSettingsSection>

    <NcSettingsSection :name="t('duplicatefinder', 'Ignore Mounted Files')"
      :description="t('duplicatefinder', 'When true, files mounted on external storage will be ignored.')" :limit-width="true">
      <NcCheckboxRadioSwitch :checked.sync="settings.ignore_mounted_files"
      @change="saveSettings('ignore_mounted_files', settings.ignore_mounted_files)">{{ t('duplicatefinder','Ignore mounted file') }}</NcCheckboxRadioSwitch>
    </NcSettingsSection>
    ...
    <NcSettingsSection :name="t('duplicatefinder', 'Disable Filesystem Events')"
      ...
      <NcCheckboxRadioSwitch :checked.sync="settings.disable_filesystem_events" @change="saveSettings('disable_filesystem_events', settings.disable_filesystem_events)">{{ t('duplicatefinder','Disable filesystem events') }}</NcCheckboxRadioSwitch>
    </NcSettingsSection>
    ...
    <NcSettingsSection :name="t('duplicatefinder', 'Background Job Cleanup Interval (seconds)')"
      ...
      <NcTextField :value.sync="settings.backgroundjob_interval_cleanup" @change="saveSettings('backgroundjob_interval_cleanup', settings.backgroundjob_interval_cleanup)"></NcTextField>
    </NcSettingsSection>
    ...
    <NcSettingsSection :name="t('duplicatefinder', 'Background Job Find Duplicates Interval (seconds)')"
      ...
      :limit-width="true">
      <NcTextField :value.sync="settings.backgroundjob_interval_find" @change="saveSettings('backgroundjob_interval_find', settings.backgroundjob_interval_find)"></NcTextField>
    </NcSettingsSection>
  </div>
</template>
